...
export class ForecastComponent extends PaginatedTableComponent {
  ...
  constructor(
    ...
    private huskyOddsSvc: HuskyOddsService,
  ...
  createForecast() {
    ...
    modalRef.componentInstance.serviceType = SERVICE_TYPES.HuskyOdds;
  ...
  createForecastMessage(data: IForecastPayload) {
    this.huskyOddsSvc.createForecast(data).subscribe({
      next: (res) => {
  ...
  sendBroadcastSMS(data: any) {
    this.huskyOddsSvc.sendSmsMessage(SERVICE_TYPES.HuskyOdds, data.message ,data.status)
      .subscribe({
        next: (response: any) => {
