...
export class ForecastComponent extends PaginatedTableComponent {
  ...
  constructor(
    ...
    private hotNumberSvc: HotNumbersService,
  ...
  createForecast() {
    ...
    modalRef.componentInstance.serviceType = SERVICE_TYPES.HotNumbers;
  ...
  createForecastMessage(data: IForecastPayload) {
    this.hotNumberSvc.createForecast(data).subscribe({
      next: (res) => {
    ...
    sendBroadcastSMS(data: any) {
      this.hotNumberSvc
        .sendSmsMessage(SERVICE_TYPES.HotNumbers, data.message, data.status)
        .subscribe({
          next: (response: any) => {
