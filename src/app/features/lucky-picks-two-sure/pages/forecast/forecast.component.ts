...
export class ForecastComponent extends PaginatedTableComponent {
  ...
  constructor(
    ...
    private LuckyPickTwoSureSvc : LuckyPickTwoSureService,
  ...
  createForecast() {
    ...
    modalRef.componentInstance.serviceType = SERVICE_TYPES.TwoSureLp;
  ...
  createForecastMessage(data: IForecastPayload) {
    this.LuckyPickTwoSureSvc.createForecast(data).subscribe({
      next: (res) => {
    ...
    sendBroadcastSMS(data: any) {
      console.log("Broadcast SMS:", data);
      this.LuckyPickTwoSureSvc.sendSmsMessage(SERVICE_TYPES.TwoSureLp, data.message, data.status)
        .subscribe({
          next: (response: any) => {
