...
export class PriceSetupComponent extends BasePrizeComponent implements OnInit {
    ...
    constructor(private LuckyPickTwoSureSvc: LuckyPickTwoSureService,
    ...
    updatePrizes(price: any): void {
      this.LuckyPickTwoSureSvc.createPrice(price[0]).subscribe({
        next: (result) => {
    ...
    loadPriceSetup() {
      this.LuckyPickTwoSureSvc.getPriceInfo(SERVICE_TYPES.TwoSureLp).subscribe({
        next: ({ result }) => {
          this.prizes = [
            {
              ...result,
              serviceType: SERVICE_TYPES.TwoSureLp,
