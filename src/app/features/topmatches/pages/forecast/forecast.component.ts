...
import { TopmatchesService } from "../../services/topmatches.service";
...
export class ForecastComponent extends PaginatedTableComponent {
  ...
  constructor(
    ...
    private topmatchesSvc: TopmatchesService,
  ...
  createForecast() {
    ...
    modalRef.componentInstance.serviceType = SERVICE_TYPES.TopMatches;
  ...
  createForecastMessage(data: IForecastPayload) {
    this.topmatchesSvc.createForecast(data).subscribe({
      next: (res) => {
  ...
  sendBroadcastSMS(data: any) {
    this.topmatchesSvc.sendSmsMessage(SERVICE_TYPES.TopMatches, data.message ,data.status)
      .subscribe({
        next: (response: any) => {
