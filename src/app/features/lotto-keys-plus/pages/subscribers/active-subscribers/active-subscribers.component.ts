...
import { LuckyBuckService } from "src/app/features/lucky-buck/services/lucky-buck.service";
...
export class ActiveSubscribersComponent extends PaginatedTableComponent {
   ...
   constructor(
      ...
      triggerSearchChange(query: string): void {
          ...
          this.currentPage = 1;
          this.lottoKeySvc.forecastHistory(
     ...
     action(event: any): void {
      ...
      if (event.action === DASHBOARD_ACTIONS.CANCEL_SUBCRIPTION) {
        ...
        this.lottoKeySvc.cancelRecurringInvoice(phone, recurringId, SERVICE_TYPES.LottoKeys).subscribe({
          next: (response) => {
