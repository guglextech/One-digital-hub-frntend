...
import { FlashscoreService } from "../../services/flashscore.service";
...
export class PriceSetupComponent extends BasePrizeComponent implements OnInit {
  ...
  constructor(
    private jobsLordSvc: FlashscoreService,
  ...
  loadPriceSetup() {
    this.jobsLordSvc.getPriceInfo(SERVICE_TYPES.Flashscore).subscribe({
      next: ({ result }) => {
        this.prizes = [
          {
            ...result,
            serviceType: SERVICE_TYPES?.Flashscore,
