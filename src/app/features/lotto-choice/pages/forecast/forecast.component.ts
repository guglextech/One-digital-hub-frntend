...
export class ForecastComponent extends PaginatedTableComponent {
  ...
  createForecast() {
    ...
    modalRef.componentInstance.serviceType = SERVICE_TYPES.LottoChoice;
  ...
  createForecastMessage(data: IForecastPayload) {
    this.lottoChoiceSvc.createForecast(data).subscribe({
      next: (res) => {
  ...
  broadcastSMS() {
    ...
    sendBroadcastSMS(data: any) {
      this.lottoChoiceSvc
        .sendSmsMessage(SERVICE_TYPES.LottoChoice, data.message, data.status)
        .subscribe({
          next: (response: any) => {
