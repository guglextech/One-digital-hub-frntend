...
export class PriceSetupComponent extends BasePrizeComponent implements OnInit {
  ...
  constructor(private moneyRowSvc: MoneyRowService,
  ...
  updatePrizes(price: any): void {
    this.moneyRowSvc.createPrice(price[0]).subscribe({
      next: (result) => {
  ...
  loadPriceSetup() {
    this.moneyRowSvc.getPriceInfo(SERVICE_TYPES.MoneyRow).subscribe({
      next: ({ result }) => {
        this.prizes = [
          {
            ...result,
            serviceType: SERVICE_TYPES?.MoneyRow,
