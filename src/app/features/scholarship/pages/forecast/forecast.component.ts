...
export class ForecastComponent extends PaginatedTableComponent {
  ...
  createForecast() {
    ...
    modalRef.componentInstance.serviceType = SERVICE_TYPES.Scholarships;
  ...
  createForecastMessage(data: IForecastPayload) {
    this.scholarshipSvc.createForecast(data).subscribe({
      next: (res) => {
  ...
  sendBroadcastSMS(data: any) {
    this.scholarshipSvc.sendSmsMessage(SERVICE_TYPES.Scholarships, data.message ,data.status)
      .subscribe({
        next: (response: any) => {
