...
import { FinanceLordService } from "../../services/finance-lord.service";
...
export class ForecastComponent extends PaginatedTableComponent {
  ...
  constructor(
    ...
    private financeLordSvc: FinanceLordService,
  ...
  createForecast() {
    ...
    modalRef.componentInstance.serviceType = SERVICE_TYPES.Finance;
  ...
  createForecastMessage(data: IForecastPayload) {
    this.financeLordSvc.createForecast(data).subscribe({
      next: (res) => {
  ...
  sendBroadcastSMS(data: any) {
    this.financeLordSvc.sendSmsMessage(SERVICE_TYPES.Finance, data.message ,data.status)
      .subscribe({
        next: (response: any) => {
