...
export class ForecastComponent extends PaginatedTableComponent {
  ...
  createForecast() {
    ...
    modalRef.componentInstance.serviceType = SERVICE_TYPES.KingSolomon;
  ...
  createForecastMessage(data: IForecastPayload) {
    this.bigMoneySvc.createForecast(data).subscribe({
      next: (res) => {
    ...
    sendBroadcastSMS(data: any) {
      this.bigMoneySvc.sendSmsMessage(SERVICE_TYPES.KingSolomon, data.message ,data.status)
        .subscribe({
          next: (response: any) => {
