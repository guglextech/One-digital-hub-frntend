...
export class ForecastComponent extends PaginatedTableComponent {
  ...
  constructor(
    ...
    private jobsalertSvc: JobsalertService,
  ...
  createForecast() {
    ...
    modalRef.componentInstance.serviceType = SERVICE_TYPES.JobAlerts;
  ...
  createForecastMessage(data: IForecastPayload) {
    this.jobsalertSvc.createForecast(data).subscribe({
      next: (res) => {
  ...
  sendBroadcastSMS(data: any) {
    this.jobsalertSvc.sendSmsMessage(SERVICE_TYPES.JobAlerts, data.message ,data.status)
      .subscribe({
        next: (response: any) => {
