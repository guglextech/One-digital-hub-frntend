...
import { LuckyBuckService } from "../../services/lucky-buck.service";
...
export class ForecastComponent extends PaginatedTableComponent {
  ...
  constructor(
    ...
    private luckyBuckSvc: LuckyBuckService,
  ...
  createForecast() {
    ...
    modalRef.componentInstance.serviceType = SERVICE_TYPES.LuckyBuck;
  ...
  createForecastMessage(data: IForecastPayload) {
    this.luckyBuckSvc.createForecast(data).subscribe({
      next: (res) => {
    ...
    sendBroadcastSMS(data: any) {
      this.luckyBuckSvc
        .sendSmsMessage(SERVICE_TYPES.LuckyBuck, data.message, data.status)
        .subscribe({
          next: (response: any) => {
