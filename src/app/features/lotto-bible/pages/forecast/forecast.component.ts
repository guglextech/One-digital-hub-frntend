...
export class ForecastComponent extends PaginatedTableComponent {
  ...
  createForecast() {
    ...
    modalRef.componentInstance.serviceType = SERVICE_TYPES.LottoBible;
  ...
  createForecastMessage(data: IForecastPayload) {
    this.lottoBibleSvc.createForecast(data).subscribe({
      next: (res) => {
  ...
  broadcastSMS() {
    ...
    sendBroadcastSMS(data: any) {
      this.lottoBibleSvc.sendSmsMessage(SERVICE_TYPES.LottoBible,data.message, data.status).subscribe({
        next: (response: any) => {
