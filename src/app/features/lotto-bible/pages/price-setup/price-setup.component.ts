...
export class PriceSetupComponent extends BasePrizeComponent implements OnInit {
  ...
  updatePrizes(price: any): void {
    this.lottoBibleSvc.createPrice(price[0]).subscribe({
      next: (result) => {
  ...
  loadPriceSetup() {
    this.lottoBibleSvc.getPriceInfo(SERVICE_TYPES.LottoBible).subscribe({
      next: ({ result }) => {
        this.prizes = [
          {
            ...result,
            serviceType: SERVICE_TYPES.LottoBible,
