...
export class ForecastComponent extends PaginatedTableComponent {
  override tableHeaders = [
     ...
     constructor(
       ...
       private vipSuperSvc: Vip2SuperSureService,
     ...
     createForecast() {
       ...
       modalRef.componentInstance.serviceType = SERVICE_TYPES.Vip2Sure;
     ...
     createForecastMessage(data: IForecastPayload) {
       this.vipSuperSvc.createForecast(data).subscribe({
         next: (res) => {
       ...
       sendBroadcastSMS(data: any) {
         this.vipSuperSvc.sendSmsMessage(SERVICE_TYPES.Vip2Sure, data.message, data.status)
           .subscribe({
             next: (response: any) => {
