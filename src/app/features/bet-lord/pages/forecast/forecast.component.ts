...
import { BetLordService } from "../../services/bet-lord.service";
...
export class ForecastComponent extends PaginatedTableComponent {
  ...
  constructor(
    ...
    private betLordSvc: BetLordService,
  ...
  createForecast() {
    ...
    modalRef.componentInstance.serviceType = SERVICE_TYPES.BetLord;
  ...
  createForecastMessage(data: IForecastPayload) {
    this.betLordSvc.createForecast(data).subscribe({
      next: (res) => {
  ...
  sendBroadcastSMS(data: any) {
    this.betLordSvc.sendSmsMessage(SERVICE_TYPES.BetLord, data.message ,data.status)
      .subscribe({
        next: (response: any) => {
