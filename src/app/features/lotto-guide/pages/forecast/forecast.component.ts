...
export class ForecastComponent extends PaginatedTableComponent {
  ...
  createForecast() {
    ...
    modalRef.componentInstance.serviceType = SERVICE_TYPES.LottoGuide;
  ...
  createForecastMessage(data: IForecastPayload) {
    this.lottoGuideSvc.createForecast(data).subscribe({
      next: (res) => {
  ...
  broadcastSMS() {
    ...
    sendBroadcastSMS(data: any) {
      this.lottoGuideSvc
        .sendSmsMessage(SERVICE_TYPES.LottoGuide, data.message, data.status)
        .subscribe({
          next: (response: any) => {
