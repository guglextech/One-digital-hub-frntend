...
export class PriceSetupComponent extends BasePrizeComponent implements OnInit {
  ...
  updatePrizes(price: any): void {
    this.motivationLordSvc.createPrice(price[0]).subscribe({
      next: (result) => {
  ...
  loadPriceSetup() {
    this.motivationLordSvc.getPriceInfo(SERVICE_TYPES.Motivation).subscribe({
      next: ({ result }) => {
        this.prizes = [
          {
            ...result,
            serviceType: SERVICE_TYPES?.Motivation,
