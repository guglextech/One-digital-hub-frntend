...
import { MotivationLordService } from "../../services/motivation-lord.service";
...
export class ForecastComponent extends PaginatedTableComponent {
  ...
  constructor(
    ...
    private motivationLordSvc: MotivationLordService,
  ...
  triggerSearchChange(query: string): void {
    ...
    this.currentPage = 1;
    this.motivationLordSvc
      .forecastHistory(
        SERVICE_TYPES.Motivation,
        query,
        this.currentPage,
        this.pageSize
      )
  ...
  createForecast() {
    ...
    modalRef.componentInstance.serviceType = SERVICE_TYPES.Motivation;
  ...
  createForecastMessage(data: IForecastPayload) {
    this.motivationLordSvc.createForecast(data).subscribe({
      next: (res) => {
  ...
  sendBroadcastSMS(data: any) {
    this.motivationLordSvc.sendSmsMessage(SERVICE_TYPES.Motivation, data.message ,data.status)
      .subscribe({
        next: (response: any) => {
