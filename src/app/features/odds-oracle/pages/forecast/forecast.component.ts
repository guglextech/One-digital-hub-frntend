...
import { OddsOracleService } from "../../services/odds-oracle.service";
...
export class ForecastComponent extends PaginatedTableComponent {
  ...
  constructor(
    ...
    private oddsOracleSvc: OddsOracleService,
  ...
  createForecast() {
    ...
    modalRef.componentInstance.serviceType = SERVICE_TYPES.OddsOracle;
  ...
  createForecastMessage(data: IForecastPayload) {
    this.oddsOracleSvc.createForecast(data).subscribe({
      next: (res) => {
  ...
  sendBroadcastSMS(data: any) {
    this.oddsOracleSvc.sendSmsMessage(SERVICE_TYPES.OddsOracle, data.message ,data.status)
      .subscribe({
        next: (response: any) => {
