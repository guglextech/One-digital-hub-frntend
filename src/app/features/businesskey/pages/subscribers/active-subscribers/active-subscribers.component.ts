...
import { BusinesskeyService } from "../../../services/businesskey.service";
...
export class ActiveSubscribersComponent extends PaginatedTableComponent {
   ...
   constructor(
      ...
      private jobsLordSvc: BusinesskeyService
     ) {
       super();
       this.fetchData();
     }
    ...
    triggerSearchChange(query: string): void {
     ...
     action(event: any): void {
      ...
      if (event.action === DASHBOARD_ACTIONS.CANCEL_SUBCRIPTION) {
        ...
        this.jobsLordSvc.cancelRecurringInvoice(phone, recurringId, SERVICE_TYPES.BusinessKey).subscribe({
          next: (response) => {
