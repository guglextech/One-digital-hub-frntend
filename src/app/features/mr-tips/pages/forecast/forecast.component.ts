...
import { LottoBibleService } from "src/app/features/lotto-bible/services/lotto-bible.service";
...
export class ForecastComponent extends PaginatedTableComponent {
   override tableHeaders = [
    ...
    constructor(
      ...
      private mrTipsSvc: MrTipsService,
    ...
    createForecast() {
      ...
      modalRef.componentInstance.serviceType = SERVICE_TYPES.MrTips;
    ...
    createForecastMessage(data: IForecastPayload) {
      this.mrTipsSvc.createForecast(data).subscribe({
        next: (res) => {
    ...
    broadcastSMS() {
     ...
     modalRef.componentInstance.buttonAction.subscribe((data: any) => {
      ...
      sendBroadcastSMS(data: any) {
        this.mrTipsSvc.sendSmsMessage(SERVICE_TYPES.MrTips, data.message, data.status)
          .subscribe({
            next: (response: any) => {
