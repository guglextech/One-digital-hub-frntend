...
export class PriceSetupComponent extends BasePrizeComponent implements OnInit {
   override title = "";
    ...
    constructor(private sureBankerSvc: SureBakerService,
    ...
    updatePrizes(price: any): void {
      this.sureBankerSvc.createPrice(price[0]).subscribe({
        next: (result) => {
    ...
    loadPriceSetup() {
      this.sureBankerSvc.getPriceInfo(SERVICE_TYPES.SureBanker).subscribe({
        next: ({ result }) => {
          this.prizes = [
            {
              ...result,
              serviceType: SERVICE_TYPES.SureBanker,
