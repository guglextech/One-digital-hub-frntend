...
export class ForecastComponent extends PaginatedTableComponent {
  ...
  createForecast() {
    ...
    modalRef.componentInstance.serviceType = SERVICE_TYPES.SureBanker;
  ...
  createForecastMessage(data: IForecastPayload) {
    this.sureBankerSvc.createForecast(data).subscribe({
      next: (res) => {
    ...
    sendBroadcastSMS(data: any) {
      this.sureBankerSvc
        .sendSmsMessage(SERVICE_TYPES.SureBanker, data.message, data.status)
        .subscribe({
          next: (response: any) => {
