...
import { LottoKeysService } from "src/app/features/lotto-keys-plus/services/lotto-keys.service";
import { PaginatedTableComponent } from "src/app/shared/components/paginated-table/paginated-table.component";
import { SureBakerService } from "../../../services/sure-baker.service";
...
export class ActiveSubscribersComponent extends PaginatedTableComponent {
   ...
   override tableHeaders = ["Mobile", "Status", "Network", "Recurring Invoice", "Created At"];
     ...
     constructor(
         ...
         private  sureBankerSvc: SureBakerService,
       ) {
         super();
         this.fetchData();
       }
      ...
      triggerSearchChange(query: string): void {
          ...
          this.currentPage = 1;
          this.sureBankerSvc.forecastHistory(
          SERVICE_TYPES.SureBanker,
          query,
          this.currentPage,
          this.pageSize
        )
       ...
       private fetchData(): void {
         ...
         this.sureBankerSvc.subscribers(
             SERVICE_TYPES.SureBanker,
       ...
       action(event: any): void {
        ...
        if (event.action === DASHBOARD_ACTIONS.CANCEL_SUBCRIPTION) {
          ...
          this.sureBankerSvc.cancelRecurringInvoice(phone, recurringId, SERVICE_TYPES.SureBanker).subscribe({
            next: (response) => {
