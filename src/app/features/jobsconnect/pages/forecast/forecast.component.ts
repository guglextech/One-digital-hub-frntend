...
export class ForecastComponent extends PaginatedTableComponent {
  ...
  createForecast() {
    ...
    modalRef.componentInstance.serviceType = SERVICE_TYPES.JobConnect;
  ...
  createForecastMessage(data: IForecastPayload) {
    this.jobsLordSvc.createForecast(data).subscribe({
      next: (res) => {
  ...
  sendBroadcastSMS(data: any) {
    this.jobsLordSvc.sendSmsMessage(SERVICE_TYPES.JobConnect, data.message ,data.status)
      .subscribe({
        next: (response: any) => {
