...
import { LoveRelationshipsLordService } from "../../services/love-relationships-lord.service";
...
export class ForecastComponent extends PaginatedTableComponent {
  ...
  constructor(
    ...
    private loveRelationshipsLordSvc: LoveRelationshipsLordService,
  ...
  createForecastMessage(data: IForecastPayload) {
    this.loveRelationshipsLordSvc.createForecast(data).subscribe({
      next: (res) => {
  ...
  sendBroadcastSMS(data: any) {
    this.loveRelationshipsLordSvc.sendSmsMessage(SERVICE_TYPES.LoveRelationships, data.message ,data.status)
      .subscribe({
        next: (response: any) => {
