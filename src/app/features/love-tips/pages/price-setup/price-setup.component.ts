...
export class PriceSetupComponent extends BasePrizeComponent implements OnInit {
  ...
  updatePrizes(price: any): void {
    this.loveRelationshipsLordSvc.createPrice(price[0]).subscribe({
      next: (result) => {
  ...
  loadPriceSetup() {
    this.loveRelationshipsLordSvc.getPriceInfo(SERVICE_TYPES.LoveRelationships).subscribe({
      next: ({ result }) => {
        this.prizes = [
          {
            ...result,
            serviceType: SERVICE_TYPES?.LoveRelationships,
