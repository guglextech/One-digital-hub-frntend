...
import { SportagaService } from "../../services/sportaga.service";
...
export class ForecastComponent extends PaginatedTableComponent {
  ...
  constructor(
    ...
    private sportagaSvc: SportagaService,
  ...
  createForecast() {
    ...
    modalRef.componentInstance.serviceType = SERVICE_TYPES.Sportaga;
  ...
  createForecastMessage(data: IForecastPayload) {
    this.sportagaSvc.createForecast(data).subscribe({
      next: (res) => {
  ...
  sendBroadcastSMS(data: any) {
    this.sportagaSvc.sendSmsMessage(SERVICE_TYPES.Sportaga, data.message ,data.status)
      .subscribe({
        next: (response: any) => {
