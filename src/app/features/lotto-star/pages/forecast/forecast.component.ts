...
import { LottoStarService } from "../../services/lotto-star.service";
...
export class ForecastComponent extends PaginatedTableComponent {
  ...
  constructor(
    ...
    private lottoStarSvc: LottoStarService,
  ...
  createForecast() {
    ...
    modalRef.componentInstance.serviceType = SERVICE_TYPES.LottoStar;
  ...
  createForecastMessage(data: IForecastPayload) {
    this.lottoStarSvc.createForecast(data).subscribe({
      next: (res) => {
  ...
  broadcastSMS() {
    ...
    sendBroadcastSMS(data: any) {
      this.lottoStarSvc.sendSmsMessage(SERVICE_TYPES.LottoStar,data.message, data.status).subscribe({
        next: (response: any) => {
